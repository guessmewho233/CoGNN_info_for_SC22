...
#include <iostream> // CoGNN
#include <unistd.h>  // CoGNN
#include <stdio.h>  // CoGNN
#include <sys/socket.h>  // CoGNN
#include <stdlib.h>  // CoGNN
#include <netinet/in.h>  // CoGNN
#include <string.h> // CoGNN
#include <arpa/inet.h>  // CoGNN
#define PORT 9010 // CoGNN
#define SIZE_SHARED_CACHE (26 * 1024UL * 1024UL * 1024UL) // CoGNN
...
class DeviceCachingAllocator {
 ...
 private:
  ...
  // CoGNN: Pre-allocated shared GPU memory
 ...
 public:
  ...
  /* CoGNN: allocate shared GPU memory */
  ...
  /* CoGNN: insert shared GPU memory to large block pool */
...
class THCCachingAllocator {
 ...
 public:
  ...
  /* CoGNN: allocate shared GPU memory (devices) */
  void allocateSharedCache() {
    int count = device_allocator.size();
    for (int i = 0; i < count; i++)
  ...
  /* CoGNN: send shared GPU memory (devices) */
  void sendSharedCache() {
    int count = device_allocator.size();
    for (int i = 0; i < count; i++)
  ...
  /* CoGNN: recv shared GPU memory (devices) */
  void recvSharedCache() {
    int count = device_allocator.size();
    for (int i = 0; i < count; i++)
  ...
  /* CoGNN: insert shared GPU memory to large block pool (devices) */
  void insertSharedCache(size_t size, size_t offset) {
    int count = device_allocator.size();
    for (int i = 0; i < count; i++)
