...
### Class
class ModelSummary():
    def __init__(self, model_name, para_cache_size, comp_cache_size, param_trans_pipe):
        """ """
        self.task_name, self.data_name, self.num_layers = model_name[0], model_name[1], int(model_name[2])
        self.para_cache_size, self.comp_cache_size = para_cache_size, comp_cache_size
    ...
    def load_model(self):
        ...
        with torch.cuda.stream(self.cuda_stream_for_parameter):
            torch.cuda.insert_shared_cache_for_computation(self.para_cache_size, 0)
        
        with torch.cuda.stream(self.cuda_stream_for_computation):
            torch.cuda.insert_shared_cache_for_computation(self.comp_cache_size, self.para_cache_size)
